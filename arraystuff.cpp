...
int main() {
	...
	int TotalRows[5]{ 0,0,0,0,0,0 };
	for (int i{ 0 }; i != 4; i++) {
		TotalRows[0] += Sales[i][0];
	}

	for (int i{ 0 }; i != 4; i++) {
		TotalRows[1] += Sales[i][1];
	}

	for (int i{ 0 }; i != 4; i++) {
		TotalRows[2] += Sales[i][2];
	}

	for (int i{ 0 }; i != 4; i++) {
		TotalRows[3] += Sales[i][3];
	}

	for (int i{ 0 }; i != 4; i++) {
		TotalRows[4] += Sales[i][4];
	}
	...
	for (int i{ 0 }; i != 5; i++) {
		cout << "Salesman " << i + 1 << ":" << setw(10) << Sales[i][0] << setw(10) << Sales[i][1] << setw(10) << Sales[i][2] << setw(10) << Sales[i][3] << setw(10) << Sales[i][4] << setw(10) << Sales[i][5] << setw(10) << /*TotalRows[i]*/  << endl;
	};
	//cout << "Total Sales: " << setw(8) << TotalCols[0] << setw(10) << TotalCols[1] << setw(10) << TotalCols[2] << setw(10); << TotalCols[3] << setw(10) << TotalCols[4] <<  endl;
